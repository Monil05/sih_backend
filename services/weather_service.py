...
def get_weather_and_soil_details(pincode: Optional[str]) -> Dict[str, Optional[object]]:
    """
    Backwards-compatible helper expected by recommendation_service.
    Returns:
      {
    ...
    try:
        # reuse existing function to get temperature/ph/moisture
        ...
        # Fetch humidity and textual conditions from OpenWeather (current weather)
        if OPENWEATHER_API_KEY:
            try:
                ...
                # keep whatever we have; mark partial if we have temperature
    ...
    return result
# filepath: c:\Users\nisar\OneDrive\Documents\agri_backend_v2\services\weather_service.py
import os
import requests
from typing import Optional, Dict, Any
